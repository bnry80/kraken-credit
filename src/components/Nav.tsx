...
import { ACCOUNT } from "../lib/margin";

const LINKS = ["Home", "Trade", "Markets", "Earn"];

function KrakenLogo() {
  // Official Kraken logo (octopus mark + wordmark) sourced from kraken.com
  return (
    <svg
      viewBox="0 0 91 15"
      className="h-[22px] w-auto fill-white"
...
export function Nav() {
  ...
  return (
    <header className="sticky top-0 z-30 bg-bg/10 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            aria-label="Search"
            className="flex h-9 w-9 items-center justify-center rounded-full text-ink-400 transition-colors hover:bg-white/[0.06] hover:text-ink-700"
          >
          ...
          <button
            ...
            className="hidden h-9 w-9 items-center justify-center rounded-full text-ink-400 transition-colors hover:bg-white/[0.06] hover:text-ink-700 sm:flex"
          >
            <Bell className="h-[18px] w-[18px]" />
          </button>
          <button
            type="button"
            className="rounded-[12px] bg-brand-500 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-600"
          >
            Deposit
          </button>
          <div className="ml-1 flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-xs font-semibold text-ink-900 ring-1 ring-white/10">
            {initials}
          </div>
        </div>
      </div>
